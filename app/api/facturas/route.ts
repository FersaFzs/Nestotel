    ...
    const items = [
      {
        ...
        quantity: Math.ceil((new Date(reservation.checkOut).getTime() - new Date(reservation.checkIn).getTime()) / (1000 * 60 * 60 * 24)),
        unitPrice: reservation.totalPrice / Math.ceil((new Date(reservation.checkOut).getTime() - new Date(reservation.checkIn).getTime()) / (1000 * 60 * 60 * 24)),
        vat: 10, // IVA reducido para hoteles en España
      }
    ];

    const total = items.reduce((sum, item) => sum + (item.quantity * item.unitPrice), 0);
    const vatTotal = total * 0.10; // 10% IVA
